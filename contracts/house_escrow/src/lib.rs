...
impl HouseEscrow {
    ...
    pub fn initialize(env: Env, admin: Address, pool: Address, fee_bps: u32) {
    ...
    pub fn provide_liquidity(env: Env, provider: Address, amount: i128) -> i128 {
        ...
        // In production: transfer XLM from provider to this contract
    ...
    /// Returns the XLM amount withdrawn.
    pub fn withdraw_liquidity(env: Env, provider: Address, shares: i128) -> i128 {
        ...
        // In production: transfer XLM back to provider
    ...
    /// Deducts protocol fee before transfer.
    pub fn pay_winner(env: Env, winner: Address, gross_amount: i128) -> i128 {
        ...
        // Update liquidity (fees stay in pool, boosting LP value)
        env.storage()
            .instance()
        ...
        // In production: token.transfer(&env.current_contract_address(), &winner, &net_amount);
...
mod test {
    ...
    fn test_liquidity_round_trip() {
        ...
        env.mock_all_auths();
        client.initialize(&admin, &pool, &50u32); // 0.5% fee

        // Deposit 100 XLM
        let shares = client.provide_liquidity(&lp, &1_000_000_000i128);
        assert_eq!(shares, 1_000_000_000);
        assert_eq!(client.get_total_liquidity(), 1_000_000_000);
    ...
    fn test_fee_collection() {
        ...
        env.mock_all_auths();
        client.initialize(&admin, &pool, &100u32); // 1% fee

        // Fund the pool
        client.provide_liquidity(&lp, &10_000_000_000i128); // 1000 XLM

        // Pay out 100 XLM gross → winner gets 99 XLM, 1 XLM goes to LP pool
        let net = client.pay_winner(&winner, &1_000_000_000i128);
        assert_eq!(net, 990_000_000i128); // 99 XLM
